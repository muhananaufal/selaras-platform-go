-- The outbox table for assessment.
--
-- Its contents are identical in every service and come from one source:
-- internal/platform/outbox/schema.sql. Eight hand-written copies would
-- drift apart slowly, and the difference would only show once an event went
-- missing in one service alone.
