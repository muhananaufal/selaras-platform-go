-- The outbox table for clinic (ADR-030).
--
-- clinic-svc has to confirm the account deletion saga like every other unit,
-- and that confirmation is an event written in the same transaction as the
-- deletion - the same lesson dashboard learned in its 0003_outbox. Its
-- contents come from internal/platform/outbox/schema.sql.
--
-- It belongs to clinic_owner like every clinic table; svc_clinic gets what
-- the relay and the confirmation need, and no more. Partitions are made by
-- cmd/partitions, which gives each one the parent's owner.
...
CREATE TABLE outbox (
    ...
    -- why: identical to the outbox of the other eight units, whose relay code is shared; widening attempts belongs in one migration for every outbox, not in one copy
...
GRANT SELECT, INSERT, UPDATE, DELETE ON outbox TO svc_clinic;
