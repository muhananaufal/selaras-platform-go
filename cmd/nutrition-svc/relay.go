...
import (
	...
	"github.com/muhananaufal/selaras-platform-go/internal/nutrition/adapter/consumer"
...
func startRelay(
	...
	if brokers == "" {
		log.Warn("KAFKA_BROKERS is not set; nutrition events will accumulate in the outbox unsent")
		return func() {}, nil
	}

	producer, err := kafka.NewProducer(kafka.Config{Brokers: brokers, ClientID: "nutrition-relay"})
	if err != nil {
		return nil, err
	}
	...
	go func() {
		if err := relay.Run(ctx); err != nil {
			log.Error("the nutrition outbox relay stopped", "error", err)
		}
	}()
...
func startResultConsumer(
	...
	if brokers == "" {
		log.Warn("KAFKA_BROKERS is not set; meal guides will never arrive")
		return func() {}, nil
	}
	...
	go func() {
		if err := results.Run(ctx); err != nil {
			log.Error("the nutrition result consumer stopped", "error", err)
		}
	}()
