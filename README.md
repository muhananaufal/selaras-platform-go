...
Tiga puluh ADR, masing-masing dengan **pembatal** — kondisi yang membuat
...
| Point-in-time recovery | WAL diarsipkan terus ke pgBackRest (RPO ≤ 60 detik saat idle); restore ke waktu T memuat yang sebelum T dan **tidak** yang sesudahnya, di CI setiap perubahan; di stack nyata: 25 pengguna pada T, 26 sesudah, restore ke T = 25, ke akhir = 26; kontrol negatif: tanpa arsip WAL, test gagal | [`docs/runbook/pitr.md`](docs/runbook/pitr.md) |
| Infrastruktur sebagai kode | OpenTofu 1.12.6 (lisensi MPL dibaca dari LICENSE) mengelola 35 objek di k3d; dari klaster kosong `task k3d:all` exit 0; di CI: plan setelah apply kosong, suntingan tangan terdeteksi sebagai drift (exit 2) dan dikembalikan apply berikutnya — 9/9 | [`docs/runbook/iac.md`](docs/runbook/iac.md) |
| Mutation testing | gremlins pada 8 paket domain: jumlah mutan yang lolos dan yang tanpa test tidak boleh naik, run dengan timeout ditolak; kontrol negatif gagal dengan nama mutannya. Temuan: flag ambang gremlins 0.6.0 diabaikan diam-diam, dan golden vector SCORE2 hanya mencakup perokok | [`docs/runbook/mutation-testing.md`](docs/runbook/mutation-testing.md) |
...
| SLO sebagai kode dan error budget | 2 SLO (ketersediaan 99 %, latensi 95 % ≤ 50 ms) → 34 aturan Sloth; 14 skenario promtool, termasuk kontrol negatif (tanpa pengecualian `Watch*` alert latensi menyala palsu; tanpa rasio berbasis event budget jadi `NaN`); gerbang freeze di CD dengan kode keluar per vonis (9 unit test); drill hidup: profile-svc dimatikan, 150 × 503/504, gerbang menolak (exit 1) dan `SelarasAvailabilitySLOBurn` page menyala | [`docs/runbook/error-budget.md`](docs/runbook/error-budget.md) |
| Rantai pasok yang bisa diperiksa | SBOM CycloneDX + provenance SLSA ditandatangani keyless (Sigstore) dan diverifikasi sebelum deploy; di CI atestasi dari workflow lain dan binary yang diubah satu byte **ditolak**; binary edge-gateway dibangun ulang di luar GitHub **identik byte demi byte** (`sha256:1bda5efb…`) dan lolos verifikasi atestasi CI | [`docs/runbook/supply-chain.md`](docs/runbook/supply-chain.md) |
...
| | |
| :--- | :--- |
| **Domain bebas library** | `internal/<unit>/domain` tidak mengimpor adapter, transport, broker, basis data, maupun unit lain, dan itu dijaga fitness function atas grafik dependensi nyata di `test/architecture` untuk ketujuh unit; penyedia LLM palsu tidak bisa menyentuh jaringan karena paket induknya tidak punya paket jaringan |
