...
| SLO sebagai kode dan error budget | 2 SLO (ketersediaan 99 %, latensi 95 % ≤ 50 ms) → 34 aturan Sloth; 14 skenario promtool, termasuk kontrol negatif (tanpa pengecualian `Watch*` alert latensi menyala palsu; tanpa rasio berbasis event budget jadi `NaN`); gerbang freeze di CD dengan kode keluar per vonis (9 unit test; kelima jalur `gate.sh` dijalankan) | [`docs/runbook/error-budget.md`](docs/runbook/error-budget.md) |
