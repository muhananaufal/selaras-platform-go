...
# backups), and a round only succeeds when the archive lists every unit
# schema AND the role file carries every svc_* role plus clinic_owner.
#
...
docker exec "$PG" psql -U admin -d selaras -v ON_ERROR_STOP=1 -qc "CREATE ROLE clinic_owner LOGIN PASSWORD 'x';"
...
check "a healthy round logs its verification" 'log .*verified .*schemas=9 .*roles=10' yes
...
# 4. The unit roles are back but clinic_owner is gone: a restore would have
#    nobody to give the clinic schema to, so the round fails as well.
docker exec "$PG" psql -U admin -d selaras -v ON_ERROR_STOP=1 -qc \
  "CREATE ROLE svc_llm LOGIN PASSWORD 'x'; DROP ROLE clinic_owner;"
out=$(backup 127.0.0.1 1 60)
check "a missing clinic_owner fails the round" '^exit=0$' no
check "that failed round leaves no file under a final name" '^file ' no
